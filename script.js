...
// SAFARİ'Yİ KANDIRACAĞIMIZ HAYALET EKRAN
let pg; 
...
class WhiteFogParticle {
    ...
    display(alpha) {
        ...
        gradient.addColorStop(0.6, `rgba(0, 0, 0, ${alpha * 0.5 / 255})`); 
...
function draw() {
    ...
    if (appState === "STORY_READY" || appState === "STORY_MAHCUP" || appState === "STORY_GERGIN" || appState === "HAYAL_KIRIKLIGI") {
        ...
        if (currentAsset) {
            ...
            // 1. ADIM: Videoyu Safari'nin görmediği hayalet ekrana (pg) çiz.
            pg.background(0); 
            pg.push();
            pg.imageMode(CENTER);
            pg.tint(255, characterAlpha * 255);
            pg.image(currentAsset, beyzImgPos.x + driftX, drawY + driftY, drawW, drawH);
            pg.pop();

            // 2. ADIM: Matematiksel olarak renkleri tersine çevir (Siyah arka planı beyaz, beyaz çizgileri siyah yap!)
            ...
            // 3. ADIM: Tertemiz olmuş ve düzelmiş videoyu MULTIPLY ile ana ekrana yapıştır. Kutu falan kalmayacak!
            push();
            blendMode(MULTIPLY);
            imageMode(CORNER);
            drawingContext.filter = 'blur(0.5px)'; // Pürüzsüzleştirme filtresi geri geldi
            image(pg, 0, 0);
            pop();
        }
    }
...
function windowResized() {
    ...
    pg.resizeCanvas(windowWidth, windowHeight); // Hayalet ekran da boyut değiştiriyor
    beyzImgPos.x = width / 2; 
}
