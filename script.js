...
// SİHRİ YAPACAĞIMIZ GİZLİ HAYALET EKRAN
let pg;
...
function draw() {
    ...
    if (appState === "INTRO" || appState === "TO_BLACK") {
        ...
        background(20, 20, 25); // HİKAYEDEKİ KOYU ARKA PLAN
    }
    ...
    if (appState === "STORY_READY" || appState === "STORY_MAHCUP" || appState === "STORY_GERGIN" || appState === "HAYAL_KIRIKLIGI") {
        ...
        if (currentAsset) {
            ...
            // ====== KÜÇÜK SİHİR NUMARASI BURADA BAŞLIYOR ======
            
            // 1. Hayalet ekranı tamamen beyaza boya ve orijinal videonu çiz.
            pg.blendMode(BLEND);
            pg.background(255); 
            pg.push();
            pg.imageMode(CENTER);
            ...
            // 2. Renkleri tersine çevir! (DIFFERENCE modu, beyazı siyaha, siyahı beyaza çevirir)
            pg.blendMode(DIFFERENCE);
            pg.fill(255);
            pg.noStroke();
            pg.rect(0, 0, pg.width, pg.height);
            // ŞU AN HAYALET EKRANDA SİMSİYAH BİR ARKA PLAN VE BEMBEYAZ BİR KARAKTER VAR!

            // 3. Düzelttiğimiz bu hayalet ekranı ana sayfaya SCREEN (Ekran) moduyla çiz!
            // SCREEN modu siyas rengi görünmez yapar, sadece beyaz çizgiler kalır.
            push();
            blendMode(SCREEN); 
            tint(255, characterAlpha * 255); // Opaklık ayarı
            imageMode(CORNER);
            image(pg, 0, 0);
            pop();
            
            // ====== SİHİR NUMARASI BİTTİ ======
        }
    }
}
...
function windowResized() {
    ...
    pg.resizeCanvas(windowWidth, windowHeight); // Hayalet ekranı da boyutlandır
    beyzImgPos.x = width / 2; 
}
